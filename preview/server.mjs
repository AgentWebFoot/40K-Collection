...
const types = { '.html': 'text/html', '.mjs': 'text/javascript', '.js': 'text/javascript', '.css': 'text/css', '.json': 'application/json', '.png': 'image/png', '.jpg': 'image/jpeg', '.webp': 'image/webp' };
...
const bridge = `<script>
  ...
  window.fetch = (input, options = {}) => {
    ...
    if (url.pathname.startsWith('/pyrrhic-war/')) return mockFetch(url.href, options);
...
http.createServer(async (req, res) => {
  try {
    ...
    name = name.replace(/^\/games\/PyrrhicWar\//, '/PyrrhicWar/');
    const target = path.resolve(root, '.' + name);
    const relative = path.relative(root, target);
    if (relative.startsWith('..') || path.isAbsolute(relative) ||
        !/^(preview|PyrrhicWar)[\\/]/.test(relative) || !types[path.extname(target).toLowerCase()]) {
      res.writeHead(404).end(); return;
    }
    let data = await readFile(target);
    if (name.startsWith('/PyrrhicWar/') && name.endsWith('.html')) data = data.toString().replace(/<head>/i, `<head>${bridge}`);
    res.writeHead(200, { 'Content-Type': types[path.extname(target).toLowerCase()], 'Cache-Control': 'no-store' });
    res.end(data);
  } catch {
    res.writeHead(404).end('File not found');
  }
}).listen(port, '127.0.0.1', () => console.log(`Pyrrhic War test page: http://127.0.0.1:${port}`));
